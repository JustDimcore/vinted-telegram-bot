...
class ProxyManager {
    ...
    static proxyFilePath = 'proxies.txt';

    static async init(maxRetries = 99, retryDelay = 5000) {
        // Running without proxies is a supported setup, and a missing file is not a
        // transient failure - retrying it would stall the startup for minutes before
        // the caller finally gets to continue without proxies.
        if (!proxy_settings.use_webshare && !fs.existsSync(this.proxyFilePath)) {
            Logger.info(`No ${this.proxyFilePath} found, continuing without proxies.`);
            return;
        }
        ...
        for (let attempt = 0; attempt < maxRetries; attempt++) {
            try {
                if (proxy_settings.use_webshare) {
                    ...
                    // Read the proxy file
                    const proxyFile = fs.readFileSync(this.proxyFilePath, 'utf8');
