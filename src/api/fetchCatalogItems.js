 ...
 * @param {Object} filters - Filters parsed from a subscription URL.
 ...
 */
export async function fetchCatalogItems({ cookie, filters = {}, per_page = 96, order = 'newest_first', domain = defaultExtension }) {
    return await executeWithDetailedHandling(async () => {
        const query = buildQuery(filters, per_page, order);
        const url = `https://www.vinted.${domain}/api/v2/catalog/items?${query.toString()}`;
